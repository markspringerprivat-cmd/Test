...
function getDuration(file) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      cleanup();
      // Timeout: Länge unbekannt, aber trotzdem weitermachen
      resolve(Infinity);
    }, 5000);
    ...
    media.onloadedmetadata = () => {
      ...
      cleanup();
      resolve(isFinite(media.duration) ? media.duration : Infinity);
    };

    media.onerror = () => {
      clearTimeout(timeout);
      cleanup();
      // Fehler beim Lesen der Metadaten – trotzdem weitermachen, FFmpeg kann es trotzdem verarbeiten
      resolve(Infinity);
    };
...
async function handleFileChange() {
  ...
  try {
    ...
    if (Number.isFinite(selectedFileDuration) && selectedFileDuration > MAX_DURATION_SECONDS + 1) {
