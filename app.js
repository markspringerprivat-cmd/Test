...
const FFMPEG_PACKAGE_WORKER = `https://cdn.jsdelivr.net/npm/@ffmpeg/ffmpeg@${FFMPEG_VERSION}/dist/esm/worker.js`;
const FFMPEG_LOCAL_WORKER_PROXY = new URL("./ffmpeg-worker-proxy.js", import.meta.url).href;
...
function installFfmpegWorkerPatch() {
  if (workerPatchInstalled) return;

  const NativeWorker = window.Worker;

  window.Worker = class LocalFfmpegWorker extends NativeWorker {
    constructor(scriptURL, options = {}) {
      const url = scriptURL && scriptURL.href ? scriptURL.href : String(scriptURL);

      // @ffmpeg/ffmpeg creates an internal Worker at dist/esm/worker.js.
      // Browsers block starting that Worker directly from cdn.jsdelivr.net when
      // the page runs on GitHub Pages. Redirect every matching ffmpeg package
      // worker URL to a same-origin proxy file committed next to app.js.
      const isFfmpegPackageWorker =
        url.includes("@ffmpeg/ffmpeg") &&
        url.includes("/dist/esm/worker.js");

      if (isFfmpegPackageWorker) {
        super(FFMPEG_LOCAL_WORKER_PROXY, { ...options, type: "module" });
        return;
      }
...
async function loadFfmpegModules() {
  if (FFmpegClass && ffmpegFetchFile && ffmpegToBlobURL) {
    return { FFmpeg: FFmpegClass, fetchFile: ffmpegFetchFile, toBlobURL: ffmpegToBlobURL };
  }

  installFfmpegWorkerPatch();
